            ...
            if (msg.role === 'assistant' && Array.isArray(msg.content)) {
                for (const block of msg.content) {
                    if (block.type === 'toolCall' && block.name === 'sessions_spawn') {
                        const args = typeof block.arguments === 'string' ? (() => { try {
...
function parseAgentSessionFiles() {
...
function parseSessionActivity(limit) {
    const items = [];
    if (!fs.existsSync(SESSIONS_DIR))
        return items;
    ...
    try {
        if (fs.existsSync(CHAT_LOG_FILE)) {
            const chatItems = JSON.parse(fs.readFileSync(CHAT_LOG_FILE, 'utf-8'));
...
function labelToReadable(label) {
    return label
        .replace(/[-_]/g, ' ')
        .replace(/\bv\d+$/i, '')
        .trim();
}
function cleanTaskSummary(taskLine, label) {
...
function extractSubagentInfo(text) {
...
function looksLikeCodeOrFileContent(text) {
    const trimmed = text.trim();
    if (/^['"]use (client|server)['"];/.test(trimmed))
        return true;
    if (/^import\s+[\{a-zA-Z]/.test(trimmed))
        return true;
    if (/^export\s+(default\s+)?/.test(trimmed))
        return true;
    if (/^(const|let|var|function|class|interface|type)\s+/.test(trimmed))
        return true;
    ...
    if (lines.length > 3) {
        ...
        if (indented / lines.length > 0.3)
            return true;
    }
    return false;
}
function extractRealError(text, toolName) {
    if (looksLikeCodeOrFileContent(text))
        return null;
    ...
    if (lower.includes('build failed') || lower.includes('compilation failed') || lower.includes('module not found') ||
        ...
        const errorLine = text.split('\n').find(l => /error/i.test(l) && !looksLikeCodeOrFileContent(l)) || firstLine;
...
function extractTags(allText, toolsUsed, taskLabels) {
    const tags = new Set();
    ...
    if (toolsUsed.has('browser') || toolsUsed.has('web_fetch') || toolsUsed.has('web_search'))
        tags.add('browser');
    if (toolsUsed.has('exec'))
        tags.add('dev');
    if (toolsUsed.has('tts'))
        tags.add('voice');
    if (toolsUsed.has('nodes'))
        tags.add('nodes');
    if (toolsUsed.has('canvas'))
        tags.add('canvas');
...
function parseSessionsForDate(targetDate) {
    ...
    for (const file of files) {
        ...
        for (const line of lines) {
            ...
            if (!entry.timestamp)
                continue;
            if (entry.timestamp.slice(0, 10) !== targetDate)
                continue;
...
app.get('/api/memory', (req, res) => {
    ...
    const files = [];
    const addFile = (filePath, category) => {
        try {
    ...
    const stream = new ReadableStream({
        start(controller) {
            ...
            const pushEvent = async (event) => {
                try {
                    if (event === 'tasks')
                        send('tasks', { todo: readTodos(), in_progress: parseSessionFiles().filter(t => t.status === 'in_progress'), done: parseSessionFiles().filter(t => t.status === 'done' || t.status === 'failed') });
                    else if (event === 'activity')
                        send('activity', parseSessionActivity(100));
                    else if (event === 'agents')
                        send('agents', parseAgentSessionFiles());
                    else if (event === 'costs')
                        send('costs', parseCosts());
                }
                catch { }
            };
