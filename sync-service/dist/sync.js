...
const SESSION_DIR = path.join(process.env.HOME || "/home/vtto", ".openclaw/agents/main/sessions");
...
async function getOrCreateAgent(name = "otto") {
    ...
    try {
        ...
        if (response.ok) {
            const agents = await parseJsonResponse(response);
    ...
    try {
        ...
        const agents = await fetch(`${SUPABASE_URL}/rest/v1/agents?name=eq.${name}&select=id`, {
            headers: {
                apikey: SUPABASE_ANON_KEY || "",
            },
        }).then((r) => r.json());
...
function findToolCall(contentArray, toolName) {
    if (!Array.isArray(contentArray))
        return null;
    for (const item of contentArray) {
                ...
                try {
                    ...
                    if (isCompleted || isFailed) {
                        ...
                        if (taskName) {
                            ...
                            const response = await supabaseFetch(url.toString());
                            if (response.ok) {
                                const tasks = await parseJsonResponse(response);
...
async function generateDailyJournals() {
    ...
    try {
        ...
        const response = await supabaseFetch(url.toString());
        if (response.ok) {
            const existing = await parseJsonResponse(response);
        ...
        const accomplishments = (tasks || []).map((t) => `✓ ${t.label}`);
